...
\section{Digital Out}
\label{sec:org20b06d2}
\begin{frame}[fragile,label={sec:org3d29bed}]{Startpunkt digitaler Output}
...
\begin{frame}[fragile,label={sec:org26af632}]{setup}
...
\begin{frame}[fragile,label={sec:org50d7a54}]{digitalWrite und Delay}
...
\begin{frame}[fragile,label={sec:orgb79c7c3}]{Andere Blink Muster}
...
\begin{frame}[label={sec:org4dfe82b}]{Schnelleres Blinken}
...
\section{Analog Out}
\label{sec:orgd604ffd}
\begin{frame}[fragile,label={sec:org48aafd7}]{analogWrite}
...
\begin{frame}[label={sec:org11388c5}]{PWM Funktionsweise: Zähler + Vergleich}
\begin{tikzpicture}
...
\begin{frame}[fragile,label={sec:orgbec52ad}]{Datentypen (2)}
 \begin{itemize}
...
\begin{frame}[label={sec:orgb6e10cb}]{PWM Frequenz}
...
\begin{frame}[fragile,label={sec:orge6e51ef}]{PWM Vorteiler: Timer 0, Pins 5 und 6}
 \begin{center}
\begin{tabular}{rrr}
...
\begin{frame}[fragile,label={sec:org39f08df}]{PWM Vorteiler: Timer 1, Pins 9 und 10}
 \begin{center}
\begin{tabular}{rrr}
Einstellung & Teiler & Frequenz\\
\hline
...
\begin{frame}[fragile,label={sec:org2665501}]{PWM Vorteiler: Timer 2, Pins 11 und 3}
 \begin{center}
\begin{tabular}{rrr}
Einstellung & Teiler & Frequenz\\
\hline
...
\begin{frame}[fragile,label={sec:orgb2ba0c5}]{Vorsicht}
...
\begin{frame}[fragile,label={sec:orge291ebb}]{analogRead}
 \texttt{analogRead(pin)}: 0-1023 für 0-5 Volt an Pin \texttt{pin}.
\end{frame}

\begin{frame}[fragile,label={sec:org3974367}]{Kombination mit analogWrite}
 \begin{minted}[]{c}
void setup() {
    pinMode(3, OUTPUT);
}
...
\begin{frame}[fragile,label={sec:orga97cd3f}]{An den PC senden}
 \begin{minted}[]{c}
void setup() {
  Serial.begin(115200);
}
...
\begin{frame}[label={sec:orge7446f5}]{Spannungsbereich}
...
\section{Bunte Dinge}
\label{sec:org35fc26a}
\begin{frame}[label={sec:orgff620d8}]{Macht Stephan}
\end{frame}

\section{Mehrere LEDs}
\label{sec:orge954ef8}
\begin{frame}[label={sec:org59a4beb}]{Diskret}
Vorteile:
\begin{itemize}
\item Einfach
...
\begin{frame}[label={sec:orgde102d7}]{Matrix}
Vorteile:
\begin{itemize}
...
\begin{frame}[label={sec:org3ef394f}]{Schieberegister}
\begin{itemize}
...
\label{sec:orgfe281d2}
\begin{frame}[label={sec:org88848f6}]{WS2812, APA102\ldots{}}
\begin{itemize}
...
\begin{frame}[label={sec:org1a181b3}]{Libraries}
\begin{itemize}
...
\begin{frame}[fragile,label={sec:orga46f9e0}]{Beispielcode}
 \begin{minted}[]{c}
...
\begin{frame}[fragile,label={sec:org3cfd356}]{Arrays}
...
\begin{frame}[fragile,label={sec:org32cd14c}]{Laufender Regenbogen}
...
\label{sec:org0be46f3}
\begin{frame}[label={sec:orgecd38ee}]{Macht Stephan}
\end{frame}

\section{Sensoren}
\label{sec:org0391330}
\begin{frame}[label={sec:orgca74202}]{Anschluss von Sensoren}
\begin{itemize}
...
\begin{frame}[label={sec:org532cf9f}]{Sensorbeispiele}
\begin{itemize}
\item Beschleunigung
\item Drehrate
\item Magnetfeld
...
\begin{frame}[fragile,label={sec:orge00c486}]{Sensoren im Arduino}
...
\end{frame}
\section{Basteln}
\label{sec:orgb1e7deb}
\begin{frame}[label={sec:org7549d92}]{Basteln}
Vorschläge?
