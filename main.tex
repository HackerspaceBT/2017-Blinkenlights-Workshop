...
\section{Digital Out}
\label{sec:org117c394}
\begin{frame}[fragile,label={sec:org5c28559}]{Startpunkt digitaler Output}
...
\begin{frame}[fragile,label={sec:org561c0dd}]{setup}
...
\begin{frame}[fragile,label={sec:orge296d77}]{digitalWrite und Delay}
...
\begin{frame}[fragile,label={sec:orge18d99b}]{Andere Blink Muster}
...
\begin{frame}[label={sec:orga2f53df}]{Schnelleres Blinken}
...
\section{Analog Out}
\label{sec:orgf3ef09a}
\begin{frame}[fragile,label={sec:org7234cb7}]{analogWrite}
...
\begin{frame}[label={sec:orge2ad78c}]{PWM Funktionsweise: Zähler + Vergleich}
\begin{tikzpicture}
...
\begin{frame}[label={sec:orgd8c2771}]{PWM, Schwellwert 128}
\begin{tikzpicture}
...
\begin{frame}[label={sec:org182e33f}]{PWM, Schwellwert 16}
\begin{tikzpicture}
\begin{axis}[xlabel=Zeit / s, ylabel=Spannung / V, ymin=-0.1, ymax=5.1]
...
\begin{frame}[fragile,label={sec:org3f7dd21}]{Einfacher PWM Code}
 \begin{minted}[]{c}
...
\begin{frame}[fragile,label={sec:orge0edc2f}]{Datentypen (1)}
 \begin{itemize}
...
\begin{frame}[fragile,label={sec:orgdef0c32}]{Datentypen (2)}
 \begin{itemize}
...
\begin{frame}[label={sec:org407cff6}]{PWM Frequenz}
...
\begin{frame}[fragile,label={sec:orgedf8568}]{PWM Vorteiler: Timer 0, Pins 5 und 6}
 \begin{center}
\begin{tabular}{rrr}
...
\begin{frame}[fragile,label={sec:orgbfdfc20}]{PWM Vorteiler: Timer 1, Pins 9 und 10}
 \begin{center}
\begin{tabular}{rrr}
Einstellung & Teiler & Frequenz\\
\hline
...
\begin{frame}[fragile,label={sec:orge477aed}]{PWM Vorteiler: Timer 2, Pins 11 und 3}
 \begin{center}
\begin{tabular}{rrr}
Einstellung & Teiler & Frequenz\\
\hline
...
\begin{frame}[fragile,label={sec:orgc63b83f}]{Vorsicht}
...
\section{Digital In}
\label{sec:orgc80df0d}
\begin{frame}[label={sec:org77d7e52}]{Startpunkt digitaler Input}
...
\begin{frame}[fragile,label={sec:org804b556}]{digitalRead}
...
\begin{frame}[label={sec:orgfce5c9c}]{Schaltplanvarianten}
\begin{center}
\includegraphics[width=.9\linewidth]{./img/pullup.jpg}
\end{center}
\end{frame}

\begin{frame}[fragile,label={sec:org3bb874c}]{Unterbrechbare Abläufe starten (1)}
 \begin{minted}[]{c}
...
\begin{frame}[fragile,label={sec:orgb069c0e}]{Unterbrechbare Abläufe starten (2)}
 \begin{minted}[]{c}
...
\begin{frame}[label={sec:org68f54be}]{Prellen}
\begin{center}
\includegraphics[width=.9\linewidth]{./img/bounce.png}
\end{center}
\end{frame}

\begin{frame}[label={sec:org05464e2}]{Entprellen}
Auch: Debouncing
...
\section{Analog In}
\label{sec:org000ff13}
\begin{frame}[label={sec:org9f79597}]{Startpunkt analoger Input}
AnalogInput Beispiel: File \(\rightarrow\) Examples \(\rightarrow\) Analog
\(\rightarrow\) AnalogInput
\end{frame}

\begin{frame}[fragile,label={sec:orgd306678}]{analogRead}
 \texttt{analogRead(pin)}: 0-1023 für 0-5 Volt an Pin \texttt{pin}.
\end{frame}

\begin{frame}[fragile,label={sec:org25f138c}]{Kombination mit analogWrite}
 \begin{minted}[]{c}
void setup() {
    pinMode(3, OUTPUT);
}
...
\begin{frame}[fragile,label={sec:org1da21d0}]{An den PC senden}
 \begin{minted}[]{c}
void setup() {
  Serial.begin(115200);
}
...
\begin{frame}[label={sec:org54d61c3}]{Spannungsbereich}
...
\section{Bunte Dinge}
\label{sec:orgb2c06f0}
\begin{frame}[label={sec:orgac0591a}]{Macht Stephan}
\end{frame}

\section{Mehrere LEDs}
\label{sec:orgd5fd37c}
\begin{frame}[label={sec:org4513488}]{Diskret}
Vorteile:
\begin{itemize}
\item Einfach
...
\begin{frame}[label={sec:org7ad65e7}]{Matrix}
Vorteile:
\begin{itemize}
...
\begin{frame}[label={sec:org431fce3}]{Schieberegister}
\begin{itemize}
...
\label{sec:org91103ee}
\begin{frame}[label={sec:orgd040075}]{WS2812, APA102\ldots{}}
\begin{itemize}
...
\begin{frame}[label={sec:orga843e69}]{Libraries}
\begin{itemize}
...
\begin{frame}[fragile,label={sec:orgbb6afba}]{Beispielcode}
 \begin{minted}[]{c}
...
\begin{frame}[fragile,label={sec:org1a60cde}]{Arrays}
...
\begin{frame}[fragile,label={sec:orgce8a086}]{Laufender Regenbogen}
...
\label{sec:org052e7c5}
\begin{frame}[label={sec:org3c0d40a}]{Macht Stephan}
\end{frame}

\section{Sensoren}
\label{sec:orgc2126ef}
\begin{frame}[label={sec:orgb8f8aa0}]{Anschluss von Sensoren}
\begin{itemize}
...
\begin{frame}[label={sec:org269d799}]{Sensorbeispiele}
\begin{itemize}
\item Beschleunigung
\item Drehrate
\item Magnetfeld
...
\begin{frame}[fragile,label={sec:orgfc228fa}]{Sensoren im Arduino}
